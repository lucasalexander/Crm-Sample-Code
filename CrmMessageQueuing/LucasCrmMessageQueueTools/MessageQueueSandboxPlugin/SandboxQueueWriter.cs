...
namespace MessageQueueSandboxPlugin
{
    public class SandboxQueueWriter : IPlugin
    {
        ...
        public SandboxQueueWriter(string config)
        {
            ...
            else
            {
                ...
                XmlNodeList nodeendpointnodes = doc.DocumentElement.SelectNodes("/config/nodeendpoint");
                if (nodeendpointnodes.Count == 1)
                {
                    _nodeEndpoint = nodeendpointnodes[0].InnerText;
                }
                else
                {
        ...
        public void Execute(IServiceProvider serviceProvider)
        {
                ...
                context.InputParameters["Target"] is Entity)
            {
                ...
                //prepare the json message that will be sent to node.js 
                ...
                //these first few properties tell node.js how to connect to rabbitmq
                jsonWriter.WritePropertyName("endpoint");
                jsonWriter.WriteValue(_brokerEndpoint);
                jsonWriter.WritePropertyName("username");
                jsonWriter.WriteValue(_brokerUser);
                jsonWriter.WritePropertyName("password");
                jsonWriter.WriteValue(_brokerPassword);
                jsonWriter.WritePropertyName("exchange");
                jsonWriter.WriteValue(_exchange);
                jsonWriter.WritePropertyName("routingkey");
                jsonWriter.WriteValue(_routingKey);

                //now add the message that will be passed on to rabbitmq
                ...
                try
                {
                    ...
                    System.Net.WebRequest req = System.Net.WebRequest.Create(_nodeEndpoint);
                    ...
                    //if the response from node.js is anything other than "success," throw an exception
                    string response = responseStream.ReadToEnd();
                    if(response.ToUpperInvariant()!="SUCCESS")
                    {
                        throw new InvalidPluginExecutionException(
                            "Error writing message to queue: " + response);
                    }
                }
